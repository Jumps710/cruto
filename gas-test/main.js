// ログシート取得ヘルパー関数（エラーハンドリング付き）
function getLogSheet() {
  try {
    const sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName("Log");
    if (!sheet) {
      console.error("Logシートが見つかりません");
      // Logシートが存在しない場合は作成
      const spreadsheet = SpreadsheetApp.getActiveSpreadsheet();
      const newSheet = spreadsheet.insertSheet("Log");
      // ヘッダー行を設定
    ...
    console.error("ログシート取得エラー:", error);
    return null;
  }
}
