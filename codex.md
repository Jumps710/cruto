...
- 環境切り替え: `gas/env.gs` の `CURRENT_ENV` を 'DEV'/'PROD' に切り替えるだけでOK。値も同ファイルで管理。
...
\n- Always run clasp push from the appropriate project directory after modifying Apps Script sources.
