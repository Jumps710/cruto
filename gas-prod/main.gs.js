function getLogSheet() {
  try {
    ...
    if (!sheet) {
      // Logシートが存在しない場合は作成
      return SpreadsheetApp.getActiveSpreadsheet().insertSheet("Log");
    }
    return sheet;
  } catch (error) {
    console.error('Logシート取得エラー:', error);
    return null;
  }
}
...
function testLogSheet() {
  ...
  if (sheet) {
    console.log("Logシート取得成功:", sheet.getName());
    // テストログを記録
    sheet.appendRow([
      new Date(),
      "TEST",
...
function doPost(e) {
  ...
  if (logSheet) {
    ...
    console.log("ログシートが利用できません - リクエスト到達:", timestamp);
  }
...
function logGoogleMapsApiResponse(requestData) {
  ...
  let source = 'unknown'; // ← スコープを関数に上げる
